...
def myMultiOpt(idx):
    simulation_idx,duration_idx=idx
    ...
    Fpath_Dataset=f'{Fpath_Dataset}/{simulation_idx}/'
    Fpath_Dataset_RRA=f'{Fpath_Dataset_RRA}/{simulation_idx}/'

    Fpath_Save=f'{Fpath_Save}/Ratio_{ratio_cost_rev}/{simulation_idx}/'
    Fpath_Save_RRA=f'{Fpath_Save_RRA}/Ratio_{ratio_cost_rev}/{simulation_idx}/'
...
for simulation in Simulations:
    for duration in duration_list:
        pair_list_duration_OP.append([simulation,duration])
    


if __name__ == '__main__':
    with Pool(len(time_decision_NP)) as p:
